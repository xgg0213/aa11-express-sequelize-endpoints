...
router.get('/', async (req, res, next) => {
    ...
    // Your code here 
    trees = await Tree.findAll({order:[['heightFt','DESC']]});
...
router.post('/', async (req, res, next) => {
    try {
        ...
        const newTree = await Tree.create({
            ...
            heightFt: height?height:null,
            groundCircumferenceFt: size?size:null
        });
...
router.put('/:id', async (req, res, next) => {
    ...
    try {
        ...
        const updateTree = await Tree.findByPk(paramId);
        const {id, name, location, height, size} = req.body;
        
        if(updateTree && paramId===id) {
            await updateTree.update({
                tree:name?name:updateTree.tree,
                location:location?location:updateTree.location,
                heightFt:height?height:updateTree.heightFt,
                groundCircumferenceFt:size?size:updateTree.groundCircumferenceFt
            })
            ...
            next({
                status:"not-found",
                message: `Could not update tree ${paramId}`,
                details:"Tree not found"
            })
        }
        else if (paramId!==id) {
            next({
                ...
                details:`${paramId} does not match ${id}`
            })
        } 
...
router.get('/search/:value', async (req, res, next) => {
    ...
    trees = await Tree.findAll({where: {[Op.like]:value}})
